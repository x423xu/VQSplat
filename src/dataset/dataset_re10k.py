...
class DatasetRE10k(IterableDataset):
    ...
    def __init__(
        ...
        for root in cfg.roots:
            root = Path(root)/Path(self.data_stage)
            root_chunks = sorted(
    ...
    def __iter__(self):
        ...
        for chunk_path in self.chunks:
            ...
            for example in chunk:
                extrinsics, intrinsics = self.convert_poses(example["cameras"]) # here we get an inverse extrinsics, and an intrinsics matrix
                scene = example["key"]
                ...
                for view_index in view_indices:
                    ...
                    if context_extrinsics.shape[0] == 2 and self.cfg.make_baseline_1:
                        ...
                        scale = (a - b).norm() # get the translation between two frames
                        if scale < self.cfg.baseline_epsilon: # translation is too small implies same contents.
                            print(
                                f"Skipped {scene} because of insufficient baseline "
                                f"{scale:.6f}"
                            )
                            continue
                        extrinsics[:, :3, 3] /= scale # the translation between two frames is normalized to 1
                    else:
                        scale = 1
